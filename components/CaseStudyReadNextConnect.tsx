...
export type CaseStudyReadNextCard = {
  href: string;
  title: string;
  imageSrc: string;
...
export type CaseStudyReadNextConnectProps = {
  /** When set, replaces the default “Security AI / Beyond Generic Alerts” pair. */
  readNextPair?: readonly [CaseStudyReadNextCard, CaseStudyReadNextCard];
};

function ReadNextCardArticle({
  card,
}: {
  card: CaseStudyReadNextCard;
}) {
  return (
    <Link
      href={card.href}
      ...
      <article>
        <div className="relative mb-5 aspect-[16/11] overflow-hidden rounded-3xl bg-neutral-900 shadow-[0_12px_40px_-20px_rgba(0,0,0,0.25)] ring-1 ring-black/5 sm:mb-6">
          <Image
            ...
            unoptimized={card.imageSrc.startsWith("/")}
          />
          {card.launched ? (
        ...
        <article>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
            <h3
              className={`${instrumentSans} min-w-0 flex-1 text-lg font-bold leading-snug tracking-tight text-neutral-950 md:text-xl`}
            >
              Beyond Generic Alerts: Boosting Premium Subscriptions with AI Event
              Captions
            </h3>
            <div className="flex shrink-0 flex-col items-start gap-2 sm:items-end">
              <Tag>AI</Tag>
      ...
      >
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-12 grid gap-10 sm:mt-14 md:grid-cols-2 md:gap-8 lg:gap-12">
            {readNextPair ? (
              <>
                <ReadNextCardArticle card={readNextPair[0]} />
                <ReadNextCardArticle card={readNextPair[1]} />
              </>
            ) : (
              <DefaultReadNextGrid />
            )}
